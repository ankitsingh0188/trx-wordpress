	...
	if ( 'entire-site' == $enable_slider  || ( ( is_front_page() || ( is_home() && $page_for_posts != $page_id ) ) && 'homepage' == $enable_slider  ) ) {
		if ( ( !$output = get_transient( 'rock_star_featured_slider' ) ) ) {
			...
			if ( $enable_slidersocial ) {
				$sliderclass = "main-slider with-social";
			}
			else {
				$sliderclass = "main-slider";
			}
			$output = '
				<section class="' . $sliderclass . '">
					<div class="wrapper">
...
function rock_star_demo_slider( $options ) {
	...
	<article class="post demo-image-1 hentry slides displayblock">
		<figure class="slider-image">
        	<img src="'.get_template_directory_uri().'/images/gallery/slider-01-1920x1080.jpg" alt="banner-image" class="banner">
        </figure><!-- .slider-image -->
    ...
    <article class="post demo-image-2 hentry slides displaynone">
    	<figure class="slider-image">
        	<img src="'.get_template_directory_uri().'/images/gallery/slider-02-1920x1080.jpg" alt="banner-image" class="banner">
        </figure><!-- .slider-image -->
        <div class="entry-container slider-contents slide-one">
            <header class="entry-header">
			...
			<article class="'.$classes.'">
				<figure class="slider-image">';
					if ( has_post_thumbnail() ) {
						...
						$rock_star_image = '<img class="pngfix wp-post-image" src="'.get_template_directory_uri().'/images/gallery/no-featured-image-1920x800.jpg" >';
				...
				<div class="entry-container slider-contents slide-one">
                    <header class="entry-header">
                        <h2 class="entry-title">><a title="Permalink to '.the_title('','',false).'" href="' . esc_url( get_permalink() ) . '">'.the_title( '<span>','</span>', false ).'</a></h2>
                    </header>
                    ';
	                if ( $excerpt !='') {
	...
	if ( !empty( $category_list ) ) {
		...
		while ( $get_featured_posts->have_posts()) : $get_featured_posts->the_post(); $i++;
			...
			<article class="'.$classes.'">
				<figure class="slider-image">';
					if ( has_post_thumbnail() ) {
						$rock_star_category_slider .= '<a title="Permalink to '.the_title('','',false).'" href="' . esc_url( get_permalink() ) . '">
		...
		if ( !empty ( $options[ 'featured_slider_image_'. $i ] ) ) {
			...
			if ( !empty ( $options['featured_content_'. $i] ) || !empty ( $options[ 'featured_link_'. $i ] ) ) {
				$content .= '<div class="entry-summary slider-btn"><p>';
					...
					if ( !empty ( $options['featured_link_'. $i] ) ) {
						$content .= '<span class="read-more"><a class="more-link btn btn-transparent" href="' . esc_url( $link ) . '">' .  sprintf( esc_html__( '%s', 'rock-star' ) , $more_tag_text ) . '</a></span>';
					}
