...
$layouts = rock_star_get_archive_content_layout();
$choices = array();
foreach ( $layouts as $layout ) {
	$choices[ $layout['value'] ] = $layout['label'];
}
...
$single_post_image_sizes = rock_star_single_post_image_size_options();
$choices = array();
foreach ( $single_post_image_sizes as $single_post_image_size ) {
	$choices[$single_post_image_size['value']] = $single_post_image_size['label'];
}
...
$navigation_description = sprintf(
	wp_kses(
		__( '<a target="_blank" href="%s">WP-PageNavi Plugin</a> is recommended for Numeric Option(But will work without it).<br/>Infinite Scroll Options requires <a target="_blank" href="%s">JetPack Plugin</a> with Infinite Scroll module Enabled.', 'rock-star' ),
		array(
			'a' => array(
				'href' => array(),
				'target' => array(),
...
$pagination_types = rock_star_get_pagination_types();
$choices = array();
